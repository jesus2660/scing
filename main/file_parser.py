    ...
    with open(filename, 'r') as data:
        ...
        for line in lines[1:len(lines)]:
            ...
            estudiante = Estudiante(nombres=tokens[1],
                                    apellidos=tokens[2],
                                    ci=tokens[3],
                                    telefono_habitacion=tokens[4],
                                    telefono_celular=tokens[5],
                                    email_ula=tokens[6],
            ...
            if not Profesor.objects.filter(ci=tokens[11]):
                ...
                profesor = Profesor(nombres = nombre,
                                    ...
                                    telefono_oficina = tokens[12],
                                    telefono_celular = tokens[13],
                                    )
                ...
                nombre_completo = profesor.nombres + profesor.apellidos
                report_data["log"].append(u"Agregado profesor \"%s %s\" (recuerde verificar si los datos son correctos y agregar los campos faltantes en la interfaz de administracion" % nombre_completo)
                profesor.save()
            ...
            if not Proyecto.objects.filter(titulo=titulo):
                ...
                if created:
                    report_data["log"].append(u"Agregada facultad \"%s\"" % facultad.nombre)
            ...
            asesor, created = Asesor.objects.get_or_create(nombres=tokens[28],email=tokens[31],telefono_institucional=tokens[29],telefono_celular=tokens[30],cargo=tokens[32],comunidad=comunidad)
            estudiante.asesor = asesor
            if created:
                report_data["log"].append("Agregado asesor comunitario \"%s\" (recuerde verificar si los datos son correctos)" % asesor.nombre)
